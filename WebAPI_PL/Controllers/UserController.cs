...
public class UserController : ControllerBase
{
    ...
    [HttpGet("findByID/{id:int}"), ValidateAntiForgeryToken]
    public async Task<ActionResult<UserMainDataDTO>> GetMainData(int id)
    {
        if (Utils.GetUserIDFromJWT(Request) != id) return BadRequest("User can get only his own data.");
    ...
    [HttpGet("findByEmail/{email}"), ValidateAntiForgeryToken]
